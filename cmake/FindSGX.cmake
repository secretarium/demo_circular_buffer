...
if(NOT WIN32)
  if(CMAKE_SIZEOF_VOID_P EQUAL 8)
    ...
    set(SGX_COMMON_CFLAGS -m64)
    set(SGX_LIBRARY_PATH ${SGX_PATH}/lib64)
    set(SGX_ENCLAVE_SIGNER ${SGX_PATH}/bin/win32/sgx_sign)
    set(SGX_EDGER8R ${SGX_PATH}/bin/win32/sgx_edger8r)
  endif()
else()
  if(CMAKE_SIZEOF_VOID_P EQUAL 8)
    set(SGX_BINARIES ${SGX_PATH}/bin/x64/Release)
    set(SGX_LIBRARY_PATH ${SGX_PATH}/bin/x64/Release)
    set(SGX_ENCLAVE_SIGNER ${SGX_BINARIES}/sgx_sign.exe)
    set(SGX_EDGER8R ${SGX_PATH}/bin/win32/Release/sgx_edger8r.exe)
  else()
...
if(SGX_FOUND)
  ...
  function(enclave_edl_library target edl edl_search_paths use_prefix edl_include_path)
    ...
    target_include_directories(${target} PUBLIC ${CMAKE_CURRENT_BINARY_DIR} PRIVATE ${INCLUDE_PATHS} ${ENCLAVE_LIBCXX_INCLUDES})
  ...
  function(host_edl_library target edl edl_search_paths use_prefix edl_include_path)
    ...
    separate_arguments(INCLUDE_PATHS)
    target_include_directories(${target} PUBLIC ${CMAKE_CURRENT_BINARY_DIR} PRIVATE ${INCLUDE_PATHS})
  ...
  function(add_enclave_library target)
    ...
    set(SGX_LINKER_FLAGS "${SGX_COMMON_CFLAGS} \
      ...
      PRIVATE  
        ...
        /DLL  
        /OPT:REF 
        #/PGD:${CMAKE_CURRENT_BINARY_DIR}/${target}.pgd 
        #/MANIFESTUAC:"level='asInvoker' uiAccess='false'" 
        #/ManifestFile:${CMAKE_CURRENT_BINARY_DIR}/${target}.dll.intermediate.manifest 
        /OPT:ICF 
        /ERRORREPORT:PROMPT 
        /NOLOGO 
        /NODEFAULTLIB 
        /TLBID:1 
        /LIBPATH:${SGX_LIBRARY_PATH})
    endif()
  ...
  function(enclave_sign target)
    ...
    if(SGX_HW AND SGX_MODE STREQUAL "Release")
      message("if(SGX_HW AND SGX_MODE STREQUAL Release")
      add_custom_target(
        ${target}-sign ALL
        COMMAND
